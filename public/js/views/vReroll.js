...
function vReroll(){
  ...
  REROLL_ATTRS.forEach(a=>{
    ...
    if(typeof IS_ADMIN==='function'&&IS_ADMIN()){html+='<button class="btn mini" style="background:linear-gradient(180deg,#ffd97a,#e0a63a)" onclick="rerollGrant(\''+a.id+'\')">👑 授予∞</button>'}
    html+='</div></div>';
  });
